...
- 同步上游属性计算器 2.2：加入完整动作结算、事件顺序假设、公式定义域校验、条件乘区和实测样本区间校准。
...
`calculator.html` 是独立的属性收益与伤害估算工具，不依赖后端或第三方脚本。当前同步至上游 2.2，支持属性收益比较、完整动作结算、五类目标、公式定义域校验、条件乘区、实测样本区间校准和一键重置；面板说明值只作为证据记录，不直接反推战斗系数。计算器默认不保存输入，刷新页面会回到初始值。
...
data/               公开的公式资料
docs/calculator/    属性计算器公式审计与控制变量记录
scripts/formula-tests.cjs  属性计算器公式回归测试
assets/             本地品牌资产与原始截图
scripts/            发布包构建脚本
